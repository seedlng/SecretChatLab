...
/**
 * Modified for extended issue-logging
 *
 * @author Valery Miller
 */

/**
 * The ExtLookup object issues queries to caching DNS servers.  The input consists
 ...
 *
 * A ExtLookup object may be reused, but should not be used by multiple threads.
 *
 * @see Cache
 * @see Resolver
 * @see ResolverConfig
...
/**
 * Create a ExtLookup object that will find records of the given name, type,
 ...
 * @see DClass
 */
public ExtLookup(Name name, int type, int dclass) {
	Type.check(type);
	DClass.check(dclass);
...
/**
 * Create a ExtLookup object that will find records of the given name and type
 * in the IN class.
 * @param name The name of the desired records
 * @param type The type of the desired records
 * @throws IllegalArgumentException The type is a meta type other than ANY.
 * @see #ExtLookup(Name,int,int)
 */
public ExtLookup(Name name, int type) {
	this(name, type, DClass.IN);
}

/**
 * Create a ExtLookup object that will find records of type A at the given name
 * in the IN class.
 * @param name The name of the desired records
 * @see #ExtLookup(Name,int,int)
 */
public ExtLookup(Name name) {
 ...
 * @see #ExtLookup(Name,int,int)
 */
public ExtLookup(String name, int type, int dclass) throws TextParseException {
 ...
 * @see #ExtLookup(Name,int,int)
 */
public ExtLookup(String name, int type) throws TextParseException {
 ...
 * @see #ExtLookup(Name,int,int)
 */
public ExtLookup(String name) throws TextParseException {
...
lookup(Name current) {
	...
	// extended logging
	LogManager.i(this, "cache: " + sr.toString());
	if (sr.isSuccessful()) {
		LogManager.i(this, "cache: " + Arrays.toString(sr.answers()));
	}
	if (verbose) {
	...
	try {
		...
		// extended logging
		if (response != null)
			LogManager.i(this, "try response: " + response.toString());
	}
	catch (IOException e) {
		// extended logging
		LogManager.i(this, "catch exception: " + e.toString());
	...
	if (!query.getQuestion().equals(response.getQuestion())) {
		...
		// !! add logging !!
		LogManager.i(this, "exception: " + badresponse_error);
		return;
	}
...
checkDone() {
	if (done && result != -1)
		return;
	StringBuffer sb = new StringBuffer("ExtLookup of " + name + " ");
	if (dclass != DClass.IN)
